...
- **[2026-07-13] セッション続行の可能性が残っているのに handoff を書くと Next が即 stale 化する**: ユーザーの「トピック列挙 + /handoff」が「別セッションへ渡す」でなく「これを見てほしい」の意図であることがある（実例: 6領域レビュー依頼 + /handoff → 直後に「コンテキスト残っているからこのまま作業しよう」となり、書いた Next Actions を同一セッションで消化）。コンテキスト残量に余裕があり依頼内容がその場で実行可能なら、journal 追記の前に「このまま本セッションで進める選択肢もあるが、区切るか？」を一言確認する。handoff は区切り確定後の最後の操作にする。
