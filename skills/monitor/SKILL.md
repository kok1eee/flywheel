...
**lite / 標的council（v0.8.42・コスト比例制御）**: loop-driver の steer 文に「lite council 可」
または「標的再council 可」が**明示されているときだけ**、以下の縮小 fan-out を選んでよい。
steer に明示が無い限り、diff が小さそうに見える等の自己判断で観測者数を減らさない（self-graded
化の再発防止・下記 Gotcha も参照）:

- **lite**: 3レンズを1つの charter に統合し `flywheel:drift-observer` を1体だけ spawn
  （「以下の3観点を1レンズとして見てください: 要件逸脱／挙動／進捗」）。
- **標的**: steer が示した lens 1つだけの charter で観測者を1体 spawn する（他レンズは前回 clean
  相当として overseer が verdict の reason に明記する）。

いずれも該当しないとき（steer に hint が無い）は下記の通常3レンズ fan-out。
...
# lite/標的 council を選んだとき（v0.8.42）は --mode を添える（省略時 full 扱い）
flywheel monitor-set clean --mode lite
flywheel monitor-set drift implementing "<reason>" --lens observer-progress --mode targeted
```

drift のときは **採用 drift を出した reviewer** を `--lens` で添える（どのレンズが効いたかの計測。
clean は採用 drift ゼロなので不要）。**Step 2 冒頭の lite/標的 hint に従って縮小 fan-out した場合は
必ず `--mode lite` / `--mode targeted` を添える**（v0.8.42・実際に回した council の重さを記録し、
lite/full の drift 捕捉率をデータ駆動で比較できるようにする。通常のフル3レンズは省略可＝ `full` 既定）。
記録は `monitor-verdicts.csv`（fw_data_dir）に溜まり、AUTO-GOTCHAS の追い出し・「レンズ別の着眼点」
昇格の判断材料になる。
...
- **[2026-07-09] lite/標的 council を自己判断で選ばない**: diff が小さい・時間が無い等の理由でモデル自身が観測者数を減らすと、FR-34 で撤去した self-graded 空通過の穴が別の形で再開く。lite/標的が許されるのは loop-driver の steer に「lite council 可」「標的再council 可」の一文が**明示されているときだけ**（Step 2 冒頭参照）。明示が無ければ通常の3レンズ fan-out を行う。
