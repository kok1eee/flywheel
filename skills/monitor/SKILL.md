...
- **[2026-06-29] 観測者は数値パース・index/範囲チェックの境界値を必ず突く**: FR-49（backlog rm/mv CLI）で council が `_backlog_int_in_range` の **前ゼロ→8進誤解釈（`08` が無効化され mv が silent にデータ破損）** を捕捉。drift だけでなく、index・行番号・range を整数解釈するコードを見たら前ゼロ / 負数 / 0 / 範囲外 / 非数字を観測者レンズに乗せる。「もっともらしく動くが境界で黙って壊れる」系は eval が緑でも通過するので、council が最後の砦になる（修正は regex `^[1-9][0-9]*$` + 回帰テスト）。
- **[2026-07-01] 観測者は「baseline は凍結値か live 参照か」を必ず突く**: FR-50 follow-up（v0.8.31・multi-repo 指紋）で council が、新設の fingerprint 計算が base を `@-`/HEAD 等の **live 参照**（`fw_baseline_rev`）から取っていたため mid-goal コミットで diff がゼロリセットする潜在バグを捕捉。base は必ず goal 開始時に確定した**凍結値**（`state.baseline_rev`）から読む必要があり、この live-baseline バグ class は新しいゲート/diff/fingerprint コードを足すたびに再発しうる。diff・指紋・累積計算を持つコードを見たら「base はどこから来るか、goal 進行中に動き得るか」を観測者レンズに乗せる。
