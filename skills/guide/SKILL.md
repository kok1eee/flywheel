...
上の `phase` で今いる場所が分かる。`no-spec`/dormant=未起動 / `designing`=設計中（実装ブロック中）/ `spec-ready`/`implementing`/`polish`/`eval`=loop が回す作業中 / `done`=達成（backlog があれば自動連鎖で次へ＝FR-33/35。空なら `flywheel next` or `reset`）。
...
実装 → 停止 → eval（自動検出 or --eval の test/lint）→ 緑 → polish(simplify) → 再 eval
      ...
      → backlog があれば自動連鎖（adopt=止めず続行 / start=連鎖前に go/no-go を grill→discovery 自動。FR-33/35・FLYWHEEL_NO_CHAIN=1 で従来 hard-stop）
...
- **goal 進行中の作業メモ** → `flywheel note "<text>"`（`.flywheel/notes.md`。greeter が最新3件を次セッションに同梱・FR-50 指紋に無影響）
- **セッションの区切り / 跨マシン引き継ぎ** → `/flywheel:handoff`（journal.md に Recap + Next）
- **dormant（goal なし）の mid-session メモ** → note は goal 進行中限定で使えない。区切るほどでなければ続行し、区切る時に handoff でまとめる
- コンテキストが十分残っているうちに handoff を書かない（journal の Next が同セッション内で stale になる）。区切るかはユーザーの判断

## flywheel の外の面との使い分け（オーケストレーション地図）

| 作業の形 | 使う面 |
|---|---|
| コード実装・機能追加（検証可能な done がある） | **flywheel**（plan route / start / adopt。eval + monitor が品質担保） |
| 戦略・分析系の一発回答（検証ループ不要） | `/flywheel:ultrawork`（全 Opus 4.8 judge panel・明示起動限定） |
| 調査・レビューの fan-out（同一ターンで集約） | Agent ツール直（binding は `run_in_background: false`・観測系は sonnet 固定） |
| self-contained な独立タスクの丸投げ / 対話が要る並行セッション | 環境側 skill（delegate-task / spawn-session 等。plugin 外＝各環境の定義が正） |
...
- **無人 chain で AskUserQuestion が恒久ブロックする（2.1.200+）**: AskUserQuestion の自動継続（idle timeout）が廃止されたため、無人運用（spawn-session の flywheel 駆動・放置 chain 等）では grill / add / discovery-council の質問が回答待ちで止まったままになる。回避は ① `/config` で idle timeout をオプトイン、② adopt 経路で判断を積む前の対話で済ませ `--notes` に焼き込んで質問レス化。対話セッションでは逆に「勝手に進まない」正しい挙動なので変更不要。
