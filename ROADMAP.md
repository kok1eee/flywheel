...
| ★★ | マルチレポ対応 — eval_cmd に sibling を含める（`uv run pytest && uv run --directory ../shared-python-lib pytest`）/ polish が両 diff を見る | 「半分しか検証されない」問題の解消 | ✅ 実装済（v0.8.9・最小スコープ）。`flywheel repos <path>...` で宣言 → diff/polish が合算。#5（cross-repo gate/昇格）は go に委譲・未実装 |
