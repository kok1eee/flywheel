...
| ★ | evolve actor-routing の機械検査 — 観測者向け Gotcha が skill 側 AUTO-GOTCHAS に誤配送されても誰も検知できない（.md 変更は eval/smoke の死角。実例: 観測者レンズ2件が monitor SKILL.md で数日死蔵・外部 /code-review が検出） | 学習 loop の「actor に届いたか」を CI で客観検証（evolve Step 2.7 の機械化） | ✅ 実装済（v0.8.32・FR-51）。`test/gotcha-actor-routing.sh`: skills/*/SKILL.md の AUTO-GOTCHAS 配下で actor 主語 title（「観測者は」・増えたら足す）を fail。実例 class 限定＝false positive ゼロ（本文言及・マーカー上は不問）。positive control / fp ガードの実走 assert 込み |
