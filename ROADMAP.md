...
| ★★ | adopt chain auto — done で backlog を自動消化（adopt は連鎖続行・start は HITL 停止・`FLYWHEEL_NO_CHAIN=1` で無効化） | 手動 `/next` を消して backlog 全部一気 | ✅ 実装済（v0.8.14・FR-33）。`test/adopt-chain.sh` で4ケース検証。無限ループ不可（pop で単調減少）・専用 cap 不要。**follow-up（v0.8.24 観測）**: 対話セッション中に done→次 goal を auto-start すると、人間が別話題に移っていた場合に goal が宙に落ちる（ROADMAP に残れば復旧可だがローカル backlog から消える＝本セッションで intent-router が実際に蒸発した）。start 経路は FR-35 で go/no-go 済みだが adopt 経路は無条件継続。adopt 経路も **着手前に軽い「次を始める?」checkpoint** を入れる候補。backlog #1「chain auto-checkpoint」の commit 粒度問題と隣接。✅ **実装済（v0.8.39・Goal C）**: 対話検知（脆い）・既定 NO_CHAIN 化（adopt chain の無人消化という価値を削る）ではなく **idle timeout 前提** を採用 — `flywheel next` を呼ぶ前に backlog 先頭の `entry` を pop せず peek し、adopt なら「次の goal に進みますか?」の軽量 AskUserQuestion checkpoint を挟む steer に変更（いいえ/あとでは backlog を pop せず温存・新規 state なし）。真に無人で回したい運用は `/config` の idle timeout オプトインに委ねる。`test/adopt-chain.sh` 更新 |
...
| ★ | stacked slash-skill（2.1.199・最大5同時ロード）の干渉 watch — `/flywheel:add ... /flywheel:next` の1プロンプト運用が可能になった一方、複数 SKILL.md のプロンプト合成でスキル間干渉（指示の衝突・steer の混線）が起き得る | flywheel skill 群の合成挙動を把握してから運用推奨を決める | 観測継続（2026-07-09 判断）。2.1.202 で**同一 skill の再 invoke 時に instructions が二重付加されるバグは host 側で修正済み**（干渉源の一部が消えた）。watch 本丸のクロス skill 合成干渉は依然未観測——実地で踏んだら実例ベースで対処の方針を維持 |
...
| ★★ | ultrawork — 原点メタプロンプトの④「常識にとらわれず全リソース総動員でトップレベルの回答」が未実装だった（①goal ②design ③grill は実現済み。eval/monitor は検証の機構であって複数案から最良を選ぶ機構ではない） | 戦略・分析系の難問に judge panel（動的lens→並列候補→審査2体→接木統合・最強可用モデル固定=現 Opus 4.8）で一発回答。メインのモデルに関係なく常に最強可用モデル品質 | ✅ 実装済（v0.8.43、**v0.8.44 で Fable 退役=2026-07-13 に伴い全 Opus 4.8 へ移行**）。`skills/ultrawork/SKILL.md`（canonical Workflow スクリプト同梱）+ `commands/ultrawork.md`。flywheel 状態機械には不接続・「ultrawork」明示トリガー限定（Opus 7〜9体の高コスト opt-in）。degrade 3系統（候補全滅→明示フォールバック/1件→judge skip/審査全滅→Synthesize 自力選定）。`test/ultrawork-skill.sh`（agent 数==opus 指定数の機械 assert・fable=退役モデルは混入検査対象・positive control 2種） |
| ★ | ultrawork の per-stage effort 調整 — Workflow の `agent()` が effort 未指定のため全体がセッション effort（既定 xhigh）を継承。Opus 4.8 の公式移行ガイドは effort sweep を推奨しており、Plan/Generate は下げても品質を保てる可能性 | panel 1回あたりのコスト削減（Opus 移行後の単価最適化。judge/synthesize は高 effort 維持） | 未着手（出所: 2026-07-13 Fable 退役日の6領域レビュー） |
| ★ | evolve に Gotchas 剪定モード — evolve は Gotchas を積む一方向で、モデル世代依存・頻度減の Gotcha が堆積する。公式ガイダンス「旧モデル向けの prescriptive な scaffolding は新モデルで品質を下げる」と正面衝突する構造 | AUTO-GOTCHAS を定期棚卸しし、無効化・一般化・削除を**人間レビュー付き**で提案（HOTL「決める=人間」維持・自動削除はしない） | 未着手（出所: 同上。Fable 5 ベストプラクティスの教訓を evolve 機構に翻訳した最重要 follow-up） |
...
| 優先 | 改善 | 効果 | 状態 |
|---|---|---|---|
| ★★ | マルチレポ対応 — eval_cmd に sibling を含める（`uv run pytest && uv run --directory ../shared-python-lib pytest`）/ polish が両 diff を見る | 「半分しか検証されない」問題の解消 | ✅ 実装済（v0.8.9・最小スコープ）。`flywheel repos <path>...` で宣言 → diff/polish/**指紋**（v0.8.31）が合算。#5（cross-repo gate/昇格）は go に委譲・未実装。jj diff path / error 経路テストは follow-up。**follow-up は v0.8.38・FR-57 で解消**（次行参照） |
| ★ | sibling .gitignore 警告 — sibling の `.flywheel` が gitignore されていないと state churn で指紋が揺れ無限 re-council し得る（FR-50 follow-up） | multi-repo 指紋 churn の入口ガード | ✅ 実装済（v0.8.38・FR-57）。repos 登録ループで `^\.flywheel/?$` を検査→未除外は理由+対処例つき stderr 警告（exit 0・登録成功＝要求でなく警告。自動追記は他リポを書き換えるので不採用）。`test/repos-gitignore-warn.sh`(C1-C3) |
...
| ★ | polish 比例制御の follow-up — ①ファイル間コード移動（追加≒削除で対称）の skip（--numstat ヒューリスティック・誤 skip リスクありで今回 decline）②`reset` の再 baseline で min-diff 閾値が無効化される件（baseline 捕捉タイミング） | FR-37 で defer した残件 | クローズ（2026-07-09 判断・痛み再発まで凍結）。①は誤 skip リスク（本物の書き換えも add≈del 対称になる）が節約を上回る——simplify のコストは FR-38 融合 + v0.8.40 の sonnet 観測者で当時より大幅に下がった。②は reset 自体が稀な核兵器で、実害は polish の誤 skip のみ（eval / monitor ゲートは非影響）。どちらも実運用で痛みが再発したら再開 |
| ★★ | monitor council のコスト比例制御 — 実測 monitor-verdicts.csv で council は1 goal あたり平均約2.4回実行（観測者は v0.8.40 で Sonnet 固定済みだが**回数**は手つかず）。drift 修正後の再検証が毎回フル3体・小さい goal でも初回からフル3体、が無駄の主因 | council の丸ごと skip はしない（self-graded 穴の再開けを回避）まま、体数・レンズ数を diff 実態に応じて絞る | ✅ 実装済（v0.8.42）。**lite council**: 初回 diff < `FLYWHEEL_MONITOR_LITE_DIFF`（既定250行）かつ watch_focus 空なら観測者1体に3レンズ統合を steer で提示。**標的再council**: drift(impl) 修正後、修正 diff が閾値未満なら指摘 lens のみ観測者1体で再検証を提示（`last_drift` に lens/level/diff_lines を退避・one-shot 消費）。安全弁3条件でフル復帰: ①watch_focus 設定時 ②直前drift が design/requirements level ③修正 diff が閾値超。lite/標的とも **steer の明示が無い限り skill 側で自己判断しない**（Gotcha 明記・self-graded 化防止）。monitor-verdicts.csv に diff_lines + mode 列を追加（6列化・旧4列/5列ヘッダは新規記録時に自動移行）し、`monitor-set --mode <lite|targeted|full>` で実際に回した council の重さを記録・lite/full の捕捉率を今後データ駆動で比較できるようにした。polish で `should_polish`/drift分岐間の diff 二重計算を解消し、hint 分岐は `monitor_hint()` に抽出（`watch_focus`+`last_drift` 併存時に one-shot クリアが素通りされるバグも同時に解消）。`test/monitor-cost-control.sh`(A1-A7 hint分岐+安全弁回帰 + B1 last_drift退避 + C1/C2 FR-38融合パス回帰)・`test/monitor-lens-csv.sh` 6列対応（C0/C0b 移行ケース + C10 --mode ケース）。**監視councilがdesignレベルdriftを1件捕捉**: FR-38融合（既定ON）が`enter_polish`側でmonitor pendingを先に確定させるため、既定設定では diff 30〜250行の通常goalでlite hintが到達不能だった構造的欠陥を`enter_polish`にも`monitor_hint()`呼び出しを追加して解消。出所: 2026-07-09 コスト削減 grill。**v0.8.46 で follow-up**: `flywheel status` に `monitor-verdicts.csv` の mode 内訳を1行表示（累積・全goal）。実データ54行（mode記録済み19件）で full=13(68%)/targeted=4/lite=2 と判明——制御は効いているがまだ過半数 full。閾値調整は次の判断として非スコープに保持。**v0.8.47 で follow-up**: full 過半数の真因は閾値ではなく、`Skill: flywheel:monitor` を Skill tool 経由で呼ぶと fork 実行になり lite/標的 hint 自体が無視されて full にフォールバックする既知バグ（v0.8.46 の goal 実行中に自ら実例を作った: diff=54行・lite可のはずが mode=full ×2件）だった。`hooks/monitor-fork-guard.sh`（PreToolUse）で eval/polish phase の該当呼出し全件を deny で機械的にブロックする対策（fork してよいケースは無いため ask ではなく deny——監視councilレビューで HOTL『人間はon-loop』原則との整合を指摘され是正）。cap 値見直し・`drift-observer` の Haiku 化は今後の判断として非スコープに保持 |
...
| 優先 | 改善 | 効果 | 状態 |
|---|---|---|---|
| ★ | 進行中文脈の軽量スナップショット（`status` と `journal` の中間）— mid-session に「今手に持っている文脈」（作業仮説 / in-flight ファイル / なぜ今これを）を安く checkpoint する薄い層。`flywheel note`（append-only scratch、次回 context に自動同梱）のような1コマンド | compact / セッション中断での now-context 揮発を防ぐ。journal を書くほどでもない区切りを拾える | ✅ 実装済（v0.8.41）。`flywheel note "<text>"`（set-eval/watch-focus と同型・goal 進行中限定＝`fw_state_exists` ガード）が `.flywheel/notes.md` に ISO8601 付き1行を追記。gitignore 済みディレクトリなので FR-50 指紋に無影響（いつ書いても re-council を誘発しない）。greeter が SessionStart で最新3件を同梱（`fw_notes_tail`）・`status` は全件表示・`fw_archive_plan` が done/次 goal 開始時に `plan/archive/<ts>/notes.md` へ退避（次 goal は空の notes.md から開始）。`test/note.sh`(C1-C5)。出所: pachitown-kb OKF 作業 2026-06-16 + 2026-07-09 Fable 5 grill |
...
| ★★ | error-fix — エラー修正モードの入口 + 教訓ゲート。失敗→教訓の回収がモデルの記憶規律任せで、ドクトリン層（memory→rule→hook）への接続点が無かった（記帳の強制はユーザー側 doctrine-nudge で解決済み・内容の生成トリガーが未整備） | 修正完了のたびに不変性フィルタ+2回ルールの昇格判定が機械的に走る。memory=無確認自動 / 恒久層昇格=人間1問（HOTL「決める=人間」の教訓層適用） | ✅ 実装済（v0.8.45）。`commands/error-fix.md`（stateless・状態機械不接続）+ `agents/debugger.md` 恒久教訓候補節 + `test/error-fix-command.sh`(C1-C5)。doctrine.md は soft-reference（タグ体系を plugin に複製しない・無ければ memory まで） |
...
| ★ | 2.1.198/199 対応 — subagent 背景デフォルト化で fork 経由 council が構造的に空振り（spawn→ターン終了→通知が親に漂着し集約・monitor-set 不実行。FR-52 で実踏）+ 2.1.191 カンマ matcher silent 失敗が示した「気づけない配線破れ」class（design-gate は fail-open） | council の同期性を明文化・hook 配線を CI で機械観測 | ✅ 実装済（v0.8.34・FR-53）。monitor Step 2 に観測者 `run_in_background: false` 明記 + Gotcha 113 root cause 追記 + 遅延漂着レポート規律（clean 後ツリー不触・improvements.md 退避）を Gotchas 化。`test/hooks-wiring.sh`（valid JSON / 全 script 実在 / カンマ matcher 禁止 / monitor sync 指示の消失検知・positive control 実走込み）。**residual**: 静的ガードは repo 側の破れのみ → host 側は **FR-54（v0.8.35）の heartbeat live positive control で部分死を検知**（全 hook 死は greeter ごと死ぬため原理的に未観測のまま＝ユーザーが完全沈黙で気づく領域） |
| ★ | hook 発火の live positive control — design-gate が発火毎に heartbeat（mtime touch）を残し、greeter が「implementing 以降なのに痕跡なし/停滞」を warn。2.1.191 型の**部分死**（greeter は生きるが特定 matcher/hook だけ無音死→設計ゲート fail-open）を検知 | host 側 fail-open の無音化対策（FR-53 residual を閉じる） | ✅ 実装済（v0.8.35・FR-54）。`fw_touch_heartbeat`（touch・サイズ無成長）+ `fw_heartbeat_staleness`（implementing/eval/polish 限定・欠如 or 7日停滞で warn・`HEARTBEAT_STALE_DAYS` 可変・observation-only）。指紋不一致 steer に遅延レポート退避の1節も追記（FR-53 altitude 指摘消化）。`test/heartbeat.sh`(C1-C5・block 経路でも痕跡が残る独立性込み) |
| ★ | fan-out skill の背景デフォルト棚卸し — 2.1.198 対応（FR-53）は monitor のみで、他の fan-out 面（discovery-council / design / verification）が同じ構造リスク（spawn→ターン終了→結果漂着）を持ったままだった | binding/advisory の判定基準を明文化し、全 binding fan-out の同期性を CI で観測 | ✅ 実装済（v0.8.36・FR-55）。binding 4 面（monitor=FR-53 済 / council 3 メンバー / designer / verification 収集子）に `run_in_background: false` 明記。council が agents/ 側の見落としを捕捉→ **`background: true` 残骸を 7 agent から除去**（2.1.196 以前の opt-in 時代の遺物。researcher + 偵察系 6）。hooks-wiring の sync ガードを 3 ファイルに拡張（reference.md は sync 数==テンプレ数の self-adjusting）。同乗: monitor-lens-csv 強化 3 点（C1b timestamp 形式 / C7 sanitize / C8 append 失敗）。出所: improvements.md |
| ★ | テスト基盤整理 — chmod→実行→戻す→assert イディオムが 3 箇所コピペ（rule-of-three）+ assert 失敗で fixture が read-only のまま残る穴 | write 失敗演習の共有化・復元保証・phase ゲート分岐の CI 網羅 | ✅ 実装済（v0.8.37・FR-56）。chain-lib に `with_readonly`（rc 透過・正常系復元）+ EXIT trap に `chmod -R u+w` 前置（異常系）の二段構え。C9 で復元保証を実走 assert。同乗: heartbeat C6-C8（spec-ready 無音 / eval / polish warn の分岐演習）+ guide Gotchas に 2.1.200 AskUserQuestion 自動継続廃止の無人運用注記 |
| ★★ | agent model tiering — Fable 5 移行で `model: inherit` の fan-out agent（監視 council 観測者 3 体等・goal 毎に必発）が上位モデルで走りコスト過大。7/3 の memory 運用ルール（spawn 時に sonnet 明示）は prompt-level で機械強制なし＝C-2 思想に反する | 観測・偵察・レビュー専任 12 agent を sonnet に静的固定・「考える部分」（analyst/designer）のみ強モデル継承。層別を CI で機械 assert（新 agent の指定忘れも検知） | ✅ 実装済（v0.8.40）。agents/ 6 ファイルを inherit→sonnet（drift-observer / architecture-mapper / code-explorer / critic / researcher / scout。debugger はユーザー判断で sonnet 維持＝難案はメインループが直接）。`test/agent-model-tiering.sh`（層別リスト照合 + リスト外検知 + positive control 実走・FR-51 同型）。per-call override（Agent ツール明示 model）は frontmatter より優先のため柔軟性は保持 |
...
- **fan-out の binding/advisory 基準（FR-55・ここが正）**: subagent の結果を**同一ターン/フローで集約して判定・成果物に使う**なら binding ＝ `run_in_background: false` を明記（2.1.198+ は背景がデフォルトで、背景 spawn だと fork/skill のターンが先に終わり結果が漂着して集約が空振りする。同一メッセージ内の並列呼び出しなら sync でも並行性は不変）。**後続の停止で拾うだけ**なら advisory ＝ 背景可。binding 4 面 = monitor 観測者 / discovery-council メンバー / design の designer / verification の証拠収集子。各 SKILL.md は結論の1行タグのみ・全文の理由はここと monitor SKILL.md Step 2。
