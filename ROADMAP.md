...
| ★★ | ultrawork — 原点メタプロンプトの④「常識にとらわれず全リソース総動員でトップレベルの回答」が未実装だった（①goal ②design ③grill は実現済み。eval/monitor は検証の機構であって複数案から最良を選ぶ機構ではない） | 戦略・分析系の難問に judge panel（動的lens→並列候補→審査2体→接木統合・全 Fable 固定）で一発回答。メインのモデルに関係なく常に Fable 品質 | ✅ 実装済（v0.8.43）。`skills/ultrawork/SKILL.md`（canonical Workflow スクリプト同梱）+ `commands/ultrawork.md`。flywheel 状態機械には不接続・「ultrawork」明示トリガー限定（Fable 7〜9体の高コスト opt-in）。degrade 3系統（候補全滅→明示フォールバック/1件→judge skip/審査全滅→Synthesize 自力選定）。`test/ultrawork-skill.sh`（agent 数==fable 指定数の機械 assert・positive control 2種） |
