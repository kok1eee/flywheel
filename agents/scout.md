...
tools: Read, Glob, Grep, WebSearch, AskUserQuestion, Agent
model: sonnet
memory: project
permissionMode: plan
...
ギャップの裏取りに広い sweep（類似実装の探索・規約確認・影響範囲の列挙）が要るときは、**Agent ツールで子に委譲**し結論だけ受け取る。自分はギャップの判定に集中する。

- 列挙系の子は `model: haiku` を明示、解釈系は指定省略（= 自分と同じモデル）
- 子の prompt には出力契約（何を返したら完了か）を明記
- 子は **`run_in_background: false`（同期）**で spawn する（親が同一ターンで結論を集約する binding fan-out。基準は ROADMAP 機構メモ・FR-55）
- 報告が期待と食い違えば継承モデルで撃ち直す。数ファイルで済むなら自分で読む
- 詳細 heuristics は `agents/capabilities.md` の「モデル選択」参照
