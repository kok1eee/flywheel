...
| `FLYWHEEL_MONITOR_LITE_DIFF` | 250 | lite council（初回 council を観測者1体3レンズ統合に軽量化）・標的再council（drift(impl) 修正後の再検証を指摘レンズのみに絞る）の安全弁閾値（累積変更行数） |
...
- **skills**: `guide`（駆動の決定ガイド・どのセッションでも）/ `grill`（plan を対話で詰問）/ `design` / `discovery-council` / `deep-interview` / `verification` / `monitor`（done-gate 検証 council・FR-30）/ `evolve` / `handoff` / `ultrawork`（全 Opus 4.8 judge panel・一発回答・v0.8.44）
...
### 0.8.43
- **ultrawork — 全 Fable 固定の judge panel スキル/コマンド** — flywheel の原点メタプロンプト「私の目的は〇〇です。最も適した指示文をあなた自身が設計してください。必要なら質問を。既存の常識にとらわれず、全リソースを総動員し、トップレベルの回答を出してください」のうち、①目的宣言=goal ②指示文の自己設計=design phase ③質問=grill は実現済みだったが、**④「常識にとらわれず全リソース総動員でトップレベルの回答」だけが未実装**だった（eval/monitor は「検証済みで正しい」の機構であって「複数案から最良を選ぶ」機構ではない）。④の実体= judge panel を独立コマンド化: `skills/ultrawork/SKILL.md`（Plan=Fable が切り口3〜5個を動的設計 → Generate=切り口ごとに Fable が候補案を並列生成 → Judge=審査2体（実行可能性/独創性の視点差）が全候補を採点+接木要素を指名 → Synthesize=勝者を骨格に接木して統合、の canonical Workflow スクリプトを同梱）+ `commands/ultrawork.md`（薄い導線）。**全 agent 呼び出しが Fable 固定**＝メインセッションのモデルが何であれ（Sonnet 運用のままでも）このコマンドの思考品質は常に Fable（v0.8.40 の subagent 層別に第3の層「判断最大化=fable」を追加する位置づけ）。**flywheel の状態機械（goal/eval/monitor/backlog）には一切乗らない**一発回答用で、対象は戦略・分析系の問いのみ（コード実装は eval ゲートの無い一発回答に不適＝flywheel:start へ誘導）。計7〜9体の Fable が走る高コスト動作のため**「ultrawork」と明示されたときだけ発動**（自発起動禁止・skill/command 起動が Workflow の明示 opt-in に該当）。degrade: 候補全滅→明示フォールバック / 候補1件→judge skip / 審査全滅→Synthesize が自力選定。`test/ultrawork-skill.sh`: 核の不変条件「SKILL.md 内の agent 呼び出し数 == fable 指定数・他モデル混入ゼロ」を CI で機械 assert（FR-51 同型・positive control 2種実走込み。実際に初回テストがプロース内のコード表記3箇所による数の汚染を正しく検出→表記分離で解消）。出所: 2026-07-09〜10 の原点メタプロンプト談義（Fable 5 で計画・実装とも）。

### 0.8.42
- **monitor council のコスト比例制御（lite council + 標的再council）** — monitor-verdicts.csv の実測: council は1 goal あたり平均約2.4回実行されており、観測者は v0.8.40 で Sonnet 固定済み（単価は下がった）だが**回数**は手つかずだった。無駄の主因は2つ: ①drift(impl) 修正後の再検証が毎回フル3体（指摘は1レンズのことが多い）、②小さい goal でも初回からフル3体。council を丸ごと skip する閾値は導入しない（2026-07-09 会話で明示的に非採用——self-graded 空通過の穴を別の形で再開けるため）。代わりに体数・レンズ数を diff 実態に応じて絞る: **lite council** は `hooks/loop-driver.sh` の monitor pending 初回分岐で `fw_goal_diff_lines` を計算し、`FLYWHEEL_MONITOR_LITE_DIFF`（既定250行）未満かつ `watch_focus` 空なら「観測者1体に3レンズ統合可」を steer で提示。**標的再council** は drift(impl) 実行分岐で `monitor null` 化の前に `last_drift`（lens/level/diff_lines）を state に退避し、次の pending 到達時に修正diffの増分が閾値未満なら「指摘レンズのみ観測者1体で再検証可」を提示（one-shot 消費）。安全弁3条件でフル復帰: ①watch_focus 設定時 ②直前drift が design/requirements level（設計やり直し後は影響面が広い） ③修正 diff が閾値超。lite/標的とも**steer に明示が無い限り skill 側の自己判断で選ばない**（`skills/monitor/SKILL.md` に Gotcha 明記・self-graded 化防止のガードレール）。あわせて `state.monitor.lens`/`mode` を新設（従来 CSV にしか無かった lens を state にも保存・標的再council の前提。mode は実際に回した council の重さ）+ `monitor-set --mode <lite|targeted|full>` を追加（省略時 full）+ `monitor-verdicts.csv` を6列化（`diff_lines`/`mode` 追加・旧4列/5列ヘッダは新規記録時に自動移行、既存データ行は不変）し、lite/full の drift 捕捉率を今後データ駆動で比較できるようにした。polish で `fw_goal_diff_lines` の重複計算（`should_polish` とmonitor gate の同ターン内・`bin/flywheel`とloop-driverのプロセス跨ぎ）を解消し、hint 分岐を `should_polish`/`monitor_bump` と対称な `monitor_hint()` 関数に抽出（`watch_focus` と `last_drift` が併存するとき one-shot クリアが素通りされていたバグも同時に解消）。`test/monitor-cost-control.sh`（A1-A7: hint 分岐・安全弁・stale化バグ回帰の単体検証・B1: 実際の drift ラウンドトリップで last_drift 退避を検証）・`test/monitor-lens-csv.sh` を6列対応に更新（C0/C0b: 旧ヘッダ移行ケース・C10: --mode 記録ケース）。**監視 council が design レベルの drift を1件捕捉**: FR-38 融合（既定 fusion ON）は `enter_polish` の融合分岐が monitor pending 初回分岐（`monitor_hint()` 呼び出し）より先に発火するため、既定設定では diff 30〜250行の通常サイズ goal で lite hint が一度も出ない構造的欠陥だった（`test/monitor-cost-control.sh` の既存ケースは全て `setup_impl` で polish を強制 skip しており、この融合パス自体を検出できない構造だった）。`enter_polish` の融合分岐内でも `monitor_hint()` を呼ぶよう修正し、融合パスを実際に通す回帰ケース（C1/C2）を追加。出所: 2026-07-09 コスト削減 grill（Fable 5 plan mode）+ 監視 council の drift(design) 捕捉。
...
### 0.8.36
- **fan-out skill の背景デフォルト棚卸し（FR-55）** — 2.1.198 の subagent 背景デフォルト化対応を monitor（FR-53）以外の fan-out 面へ展開。判定基準を明文化: **binding**（結果を同一ターン/フローで集約して判定・成果物に使う）は `run_in_background: false` を明記、**advisory**（後続の停止で拾うだけ）は背景可。棚卸し結果、binding は 4 面 — monitor 観測者（FR-53 済）/ **discovery-council メンバー 3 体**（SendMessage 相互検証→集約→曖昧点確認が同一フロー。SKILL.md に方針 + reference.md の 3 テンプレに明記）/ **design の designer**（design.md 生成を待って validate）/ **verification の証拠収集子**（親が同一ターンで照合判定）。advisory な fan-out は現 skill に無し（将来の継続監視等はこちら）。`test/hooks-wiring.sh` の sync ガードを 3 ファイルへ拡張（存在 assert・テンプレ数の増減で brittle にしない）。**同乗**: monitor-lens-csv テスト網強化 3 点（FR-52 council low note 消化）— C1b timestamp の ISO8601 形式 assert / C7 sanitize（引用符除去・カンマ→パイプ）経路の実走 / C8 既存 CSV への append 失敗（read-only ファイル）でも verdict 成功。出所: improvements.md 2026-07-03。
...
### 0.8.32
- **evolve actor-routing の機械検査（FR-51・誤配送 Gotcha の恒久ガード）** — evolve の Gotcha 書き込みは actor-routing 規則（Step 2.7・2026-07-02）を得たが **prompt-level 止まり**で、実際に観測者向けレンズ 2 件（境界値 / baseline 凍結）が `skills/monitor/SKILL.md` の AUTO-GOTCHAS に誤配送され、観測者（`agents/drift-observer.md` = system prompt）に届かないまま数日生存した。検出したのも flywheel 自身でなく外部 /code-review ＝ **非 runnable な .md 変更は eval も smoke も掛からない検証の死角**。C-2 の思想（モデルを信用せず機械が観測）を evolve の出力に適用し、恒久ガード `test/gotcha-actor-routing.sh` を CI（run-all）に追加: `skills/*/SKILL.md` の AUTO-GOTCHAS マーカーより下で **bullet title が actor 主語**（「観測者は」「reviewer は」・fan-out agent が増えたら足す）で始まる行を fail し、`agents/<name>.md` への移設を指示。**実例 class に絞った false positive ゼロ設計**（本文中の言及・マーカーより上の手動 Gotchas は不問）。lint が一度も fire しないまま self-graded 化するのを防ぐため、mktemp fixture で **positive control（誤配送を検出して非ゼロ）と false-positive ガードの実走**を同テスト内で assert（`run-all-aggregation` の前例踏襲）。出所: /code-review xhigh 指摘 → actor-routing 出荷の follow-up。
