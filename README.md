...
### 0.8.32
- **evolve actor-routing の機械検査（FR-51・誤配送 Gotcha の恒久ガード）** — evolve の Gotcha 書き込みは actor-routing 規則（Step 2.7・2026-07-02）を得たが **prompt-level 止まり**で、実際に観測者向けレンズ 2 件（境界値 / baseline 凍結）が `skills/monitor/SKILL.md` の AUTO-GOTCHAS に誤配送され、観測者（`agents/drift-observer.md` = system prompt）に届かないまま数日生存した。検出したのも flywheel 自身でなく外部 /code-review ＝ **非 runnable な .md 変更は eval も smoke も掛からない検証の死角**。C-2 の思想（モデルを信用せず機械が観測）を evolve の出力に適用し、恒久ガード `test/gotcha-actor-routing.sh` を CI（run-all）に追加: `skills/*/SKILL.md` の AUTO-GOTCHAS マーカーより下で **bullet title が actor 主語**（初期値「観測者は」・fan-out agent が増えたら足す）で始まる行を fail し、`agents/<name>.md` への移設を指示。**実例 class に絞った false positive ゼロ設計**（本文中の言及・マーカーより上の手動 Gotchas は不問）。lint が一度も fire しないまま self-graded 化するのを防ぐため、mktemp fixture で **positive control（誤配送を検出して非ゼロ）と false-positive ガードの実走**を同テスト内で assert（`run-all-aggregation` の前例踏襲）。出所: /code-review xhigh 指摘 → actor-routing 出荷の follow-up。
