...
- **skills**: `guide`（駆動の決定ガイド・どのセッションでも）/ `grill`（plan を対話で詰問）/ `design` / `discovery-council` / `deep-interview` / `verification` / `monitor`（done-gate 検証 council・FR-30）/ `evolve` / `handoff` / `ultrawork`（全 Fable judge panel・一発回答・v0.8.43）
